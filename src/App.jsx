...
export default function App() {
  ...
  function calcIncentive(r) { 
    let t = 0; 
    if (r.raksul_id_status === '提案⇒開設') t += 3000; 
  ...
  const stats = useMemo(() => {
    ...
    const idOpened = records.filter(r => r.raksul_id_status === '提案⇒開設').length;
    ...
    const staffStats = STAFF_LIST.map(s => {
      ...
      return { name: s, visits: sr.length, ids: sr.filter(r => r.raksul_id_status === '提案⇒開設').length, incentive: sr.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(s => s.visits > 0);
    const officeStats = OFFICE_LIST.map(o => {
      const or = records.filter(r => r.office === o);
      return { name: o, visits: or.length, ids: or.filter(r => r.raksul_id_status === '提案⇒開設').length, incentive: or.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(o => o.visits > 0);
                      ...
                      {records.map(r=>(
                        <tr key={r.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'14px 10px'}}>
                            <div>{(r.raksul_id_status==='提案⇒開設'||r.raksul_id_status==='開設済みだった')&&<span style={{padding:'4px 8px',background:r.raksul_id_status==='提案⇒開設'?'#dcfce7':'#f3f4f6',color:r.raksul_id_status==='提案⇒開設'?'#166534':'#64748b',borderRadius:'6px',fontSize:'11px'}}>{r.raksul_id_status==='提案⇒開設'?'開設🎉':'既存'}</span>}</div>
