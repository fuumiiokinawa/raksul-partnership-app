...
export default function App() {
  ...
  async function handleSubmit() {
    ...
    // モール購入以外の場合は理由必須
    if (formData.result_mall && formData.result_mall !== '-' && formData.result_mall !== '契約' && !formData.mall_not_purchased_reason) {
      alert('モールで購入以外を選択した場合は「購入に至らなかった理由」の入力が必須です');
      return;
    }
              ...
              <div>
                ...
                <div style={{display:'grid',gap:'12px'}}>
                  {PRODUCTS.map(p=>{
                    ...
                    // モールの場合は内諾・トスアップを無効化
                    const disabledResults = p.id === 'mall' ? ['内諾', 'トスアップ'] : [];
                    return (
                    <div key={p.id} style={{padding:'14px',background:isSelected?'#f8fafc':'#fef2f2',borderRadius:'10px',border:isSelected?`2px solid ${RESULT_COLORS[formData[`result_${p.id}`]]?.text||p.color}20`:'2px solid #fca5a5'}}>
                      ...
                      {/* モール：購入以外の場合の理由入力 */}
                      {p.id === 'mall' && formData.result_mall && formData.result_mall !== '-' && formData.result_mall !== '契約' && (
                        <div style={{marginTop:'10px'}}>
