...
export default function App() {
  ...
  }, [records, filterDateFrom, filterDateTo, filterStaff, filterOffice, filterProduct]);

  const stats = useMemo(() => {
    const totalVisits = filteredRecords.length;
    const idOpened = filteredRecords.filter(r => r.raksul_id_status === '提案⇒開設' || r.raksul_id_status === '開設済').length;
    const totalTossups = ALL_PRODUCTS.reduce((sum, p) => sum + filteredRecords.filter(r => r[`result_${p.id}`] === 'トスアップ').length, 0);
    const productStats = ALL_PRODUCTS.map(p => {
      const proposed = filteredRecords.filter(r => r[`proposal_${p.id}`] === '○').length;
      const contracts = filteredRecords.filter(r => r[`result_${p.id}`] === '契約').length;
      const tossups = filteredRecords.filter(r => r[`result_${p.id}`] === 'トスアップ').length;
      const ngs = filteredRecords.filter(r => r[`result_${p.id}`] === 'NG').length;
    ...
    const totalIncentive = filteredRecords.reduce((sum, r) => sum + calcIncentive(r), 0);
    const staffStats = STAFF_LIST.map(s => {
      const sr = filteredRecords.filter(r => r.staff === s);
      const proposed = ALL_PRODUCTS.reduce((sum, p) => sum + sr.filter(r => r[`proposal_${p.id}`] === '○').length, 0);
      const tossups = ALL_PRODUCTS.reduce((sum, p) => sum + sr.filter(r => r[`result_${p.id}`] === 'トスアップ').length, 0);
      const contracts = ALL_PRODUCTS.reduce((sum, p) => sum + sr.filter(r => r[`result_${p.id}`] === '契約').length, 0);
      return { name: s, visits: sr.length, ids: sr.filter(r => r.raksul_id_status === '提案⇒開設' || r.raksul_id_status === '開設済').length, proposed, tossups, contracts, proposalRate: sr.length > 0 ? (proposed / (sr.length * ALL_PRODUCTS.length) * 100).toFixed(1) : '0', tossupRate: proposed > 0 ? (tossups/proposed*100).toFixed(1) : '0', contractRate: proposed > 0 ? (contracts/proposed*100).toFixed(1) : '0', incentive: sr.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(s => s.visits > 0);
    const officeStats = OFFICE_LIST.map(o => {
      const or = filteredRecords.filter(r => r.office === o);
      const proposed = ALL_PRODUCTS.reduce((sum, p) => sum + or.filter(r => r[`proposal_${p.id}`] === '○').length, 0);
      const tossups = ALL_PRODUCTS.reduce((sum, p) => sum + or.filter(r => r[`result_${p.id}`] === 'トスアップ').length, 0);
      const contracts = ALL_PRODUCTS.reduce((sum, p) => sum + or.filter(r => r[`result_${p.id}`] === '契約').length, 0);
      return { name: o, visits: or.length, ids: or.filter(r => r.raksul_id_status === '提案⇒開設' || r.raksul_id_status === '開設済').length, proposed, tossups, contracts, proposalRate: or.length > 0 ? (proposed / (or.length * ALL_PRODUCTS.length) * 100).toFixed(1) : '0', tossupRate: proposed > 0 ? (tossups/proposed*100).toFixed(1) : '0', contractRate: proposed > 0 ? (contracts/proposed*100).toFixed(1) : '0', incentive: or.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(o => o.visits > 0);
    const ngStatsByProduct = ALL_PRODUCTS.map(p => {
      const reasons = filteredRecords.map(r => r[`ng_${p.id}`]).filter(Boolean);
      const counts = {};
    ...
    const videoOrdered = filteredRecords.filter(r => r.video_ordered === '申込済').length;
  ...
  function exportCSV() {
    ...
    const rows = filteredRecords.map(r => [r.visit_date,r.staff,r.company,r.industry,r.office,r.sales_source,r.sales_method,r.timing,r.raksul_id_status,r.raksul_email,r.proposal_bank,r.proposal_pay,r.proposal_mall,r.proposal_meo,r.proposal_video,r.result_bank,r.result_pay,r.result_mall,r.result_meo,r.result_video,r.ng_bank,r.ng_pay,r.ng_mall,r.ng_meo,r.ng_video,r.mall_not_purchased_reason,r.id_not_opened_reason,r.video_ordered,calcIncentive(r),r.note]);
                    ...
                    <tbody>
                      {stats.staffStats.map(s=>(
                        <tr key={s.name} style={{borderBottom:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600'}}>{s.proposed}</td>
                          <td style={{padding:'12px 8px',textAlign:'center',background:'#eff6ff',fontWeight:'700',color:'#2563eb'}}>{s.proposalRate}%</td>
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600',color:'#059669'}}>{s.contracts}</td>
                          <td style={{padding:'12px 8px',textAlign:'center',background:'#dcfce7',fontWeight:'700',color:'#059669'}}>{s.contractRate}%</td>
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600',color:'#1e40af'}}>{s.tossups}</td>
                    ...
                    <tbody>
                      {stats.officeStats.map(o=>(
                        <tr key={o.name} style={{borderBottom:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600'}}>{o.proposed}</td>
                          <td style={{padding:'12px 8px',textAlign:'center',background:'#eff6ff',fontWeight:'700',color:'#2563eb'}}>{o.proposalRate}%</td>
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600',color:'#059669'}}>{o.contracts}</td>
                          <td style={{padding:'12px 8px',textAlign:'center',background:'#dcfce7',fontWeight:'700',color:'#059669'}}>{o.contractRate}%</td>
                          <td style={{padding:'12px 8px',textAlign:'center',fontWeight:'600',color:'#1e40af'}}>{o.tossups}</td>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',zIndex:200}} onClick={()=>setSelectedProduct(null)}>
          <div style={{background:'#fff',borderRadius:'16px',width:'100%',maxWidth:'500px',maxHeight:'90vh',overflow:'auto'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:'20px'}}>
              {/* 実績データ */}
              {(() => {
                const productStat = stats.productStats.find(p => p.id === selectedProduct.id);
                if (productStat) {
                  return (
                    <div style={{marginBottom:'16px',padding:'14px',background:'linear-gradient(135deg,#1e293b,#334155)',borderRadius:'10px'}}>
                      <div style={{fontSize:'12px',color:'#94a3b8',marginBottom:'10px'}}>📊 実績データ</div>
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px'}}>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>提案</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#fff'}}>{productStat.proposed}</div>
                        </div>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>提案率</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#60a5fa'}}>{productStat.proposalRate}%</div>
                        </div>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>契約</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#34d399'}}>{productStat.contracts}</div>
                        </div>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>成約率</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#34d399'}}>{productStat.contractRate}%</div>
                        </div>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>トスアップ</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#a78bfa'}}>{productStat.tossups}</div>
                        </div>
                        <div style={{textAlign:'center'}}>
                          <div style={{fontSize:'11px',color:'#94a3b8'}}>トスアップ率</div>
                          <div style={{fontSize:'20px',fontWeight:'700',color:'#a78bfa'}}>{productStat.tossupRate}%</div>
                        </div>
                      </div>
                    </div>
                  );
