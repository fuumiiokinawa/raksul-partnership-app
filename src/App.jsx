...
const PRODUCT_DETAILS = [
  { id: 'bank', name: 'ラクスルバンク', tagline: '振込手数料119円', color: '#2563EB', icon: '🏦', url: 'https://lp-bank.raksul.com/', restricted: true, restrictedNote: '⚠️ 金融商品のため営業案内不可（紹介のみ）', features: [{ label: '振込手数料', value: '119円', highlight: true },{ label: 'ポイント還元', value: '2%', highlight: true },{ label: '口座開設', value: '最短翌日' },{ label: 'キャンペーン', value: '1万円', highlight: true }], target: '創業間もない企業', salesPoint: '地銀の振込手数料は500円前後。年間100回振込なら約4万円の差額', incentive: '契約: ¥10,000' },
  { id: 'pay', name: 'ラクスルPay', tagline: '手数料5%', color: '#059669', icon: '💳', url: 'https://rpay.raksul.com/', docUrl: PAY_DOC_URL, features: [{ label: '決済手数料', value: '5.0%', highlight: true },{ label: 'ページ開設', value: '最短10分', highlight: true },{ label: '初期/月額', value: '0円', highlight: true },{ label: '入金速度', value: '5営業日' }], target: 'オンライン販売', salesPoint: '3つの売り方', incentive: '契約: ¥10,000' },
  { id: 'mall', name: 'ビジネスモール', tagline: 'オフィス用品が安い', color: '#D97706', icon: '🛒', url: 'https://stockroom.raksul.com/', features: [{ label: 'コピー用紙', value: '業界最安級', highlight: true },{ label: '初回割引', value: '半額', highlight: true },{ label: '品揃え', value: '10万点以上' },{ label: 'ポイント', value: '貯まる' }], target: '全ての法人', salesPoint: 'コピー用紙は必ず買う', incentive: '購入: ¥7,000' },
  { id: 'meo', name: 'MEO対策', tagline: 'Googleマップ上位表示', color: '#7C3AED', icon: '📍', docUrl: MEO_DOC_URL, tossupUrl: MEO_TOSSUP_URL, videos: [{name:'HPセット販売',url:'https://drive.google.com/file/d/1KhMfd8E-8Jab1MeogEIR6SF30Od5fNsh/preview'},{name:'不動産事例',url:'https://drive.google.com/file/d/18YqR-2-6x4eSFmG7mat_uvQTWOhw_Ivw/preview'},{name:'塾事例',url:'https://drive.google.com/file/d/1wR77lpmFr2apHUnVFRuLnNuVE--8fdsH/preview'}], features: [{ label: 'セルフプラン', value: '1万円/月' },{ label: 'プロプラン', value: '4万円/月', highlight: true },{ label: '対策内容', value: '多数' },{ label: '他社比較', value: '安め', highlight: true }], target: '店舗ビジネス', salesPoint: '明確なニーズあり', incentive: '契約: ¥10,000' },
  { id: 'video', name: '出張動画撮影', tagline: '5万円で撮影', color: '#DC2626', icon: '🎬', url: 'https://st.raksul.com/web-marketing/content', features: [{ label: '撮影費用', value: '5万円', highlight: true },{ label: '納品', value: '写真+動画' },{ label: '用途', value: '多数' },{ label: '月額', value: '500円〜', highlight: true }], target: '素材がない企業', salesPoint: '写真ないですよね？', incentive: '契約: ¥10,000' },
  { id: 'raksul_id', name: 'ラクスルID', tagline: 'まず登録', color: '#6366F1', icon: '🆔', url: 'https://raksul.com/', features: [{ label: '開設報酬', value: '¥3,000', highlight: true },{ label: '登録', value: '無料', highlight: true },{ label: '印刷', value: '業界最安' },{ label: 'ポイント', value: '貯まる' }], target: '全ての訪問先', salesPoint: '全サービス登録してもらう', incentive: '開設: ¥3,000' }
];
    ...
    sections: [
      {
        subtitle: '取扱商材一覧',
        items: [
          '🏦 ラクスルバンク：振込手数料119円の法人口座（⚠️営業案内不可・紹介のみ）',
...
export default function App() {
  ...
  function exportCSV() {
    const h = ['訪問日','担当者','企業名','業種','事務所','提案タイミング','ID状態','メールアドレス','バンク提案','ペイ提案','モール提案','MEO提案','動画提案','バンク結果','ペイ結果','モール結果','MEO結果','動画結果','バンクNG','ペイNG','モールNG','MEONG','動画NG','報酬','備考'];
    const rows = records.map(r => [r.visit_date,r.staff,r.company,r.industry,r.office,r.timing,r.raksul_id_status,r.raksul_email,r.proposal_bank,r.proposal_pay,r.proposal_mall,r.proposal_meo,r.proposal_video,r.result_bank,r.result_pay,r.result_mall,r.result_meo,r.result_video,r.ng_bank,r.ng_pay,r.ng_mall,r.ng_meo,r.ng_video,calcIncentive(r),r.note]);
  ...
  const stats = useMemo(() => {
    ...
    const totalTossups = PRODUCTS.reduce((sum, p) => sum + records.filter(r => r[`result_${p.id}`] === 'トスアップ').length, 0);
    ...
    return { totalVisits, idOpened, totalTossups, productStats, staffStats, officeStats, totalIncentive, ngStatsByProduct };
  }, [records]);
        ...
        {activeTab === 'dashboard' && (
          <div style={{display:'grid',gap:'16px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:'10px'}}>
              ...
              <div style={{background:'#fff',borderRadius:'10px',border:'1px solid #e2e8f0',padding:'14px'}}><div style={{fontSize:'11px',color:'#64748b'}}>モール契約</div><div style={{fontSize:'22px',fontWeight:'700',color:'#d97706'}}>{stats.productStats.find(p=>p.id==='mall')?.contracts||0}<span style={{fontSize:'12px',color:'#94a3b8'}}>件</span></div></div>
            ...
            {stats.staffStats.length > 0 && (
              <div style={{background:'linear-gradient(135deg,#fef3c7,#fde68a)',borderRadius:'12px',padding:'16px',border:'2px solid #f59e0b'}}>
                <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
                  <span style={{fontSize:'20px'}}>🏆</span>
                  <span style={{fontSize:'15px',fontWeight:'700',color:'#92400e'}}>個人ランキング（総報酬順）</span>
                </div>
                <div style={{display:'grid',gap:'8px'}}>
                  {[...stats.staffStats].sort((a,b)=>b.incentive-a.incentive).map((s,i)=>(
                    <div key={s.name} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 14px',background:i===0?'linear-gradient(135deg,#fbbf24,#f59e0b)':i===1?'linear-gradient(135deg,#e5e7eb,#d1d5db)':i===2?'linear-gradient(135deg,#fed7aa,#fdba74)':'#fff',borderRadius:'10px',border:i<3?'none':'1px solid #e2e8f0'}}>
                      <div style={{width:'32px',height:'32px',borderRadius:'50%',background:i===0?'#fff':i===1?'#fff':i===2?'#fff':'#f3f4f6',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:'800',fontSize:i<3?'16px':'14px',color:i===0?'#f59e0b':i===1?'#6b7280':i===2?'#ea580c':'#64748b'}}>{i+1}</div>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:'700',fontSize:'15px',color:i<3?'#fff':'#1e293b'}}>{s.name}</div>
                        <div style={{fontSize:'11px',color:i<3?'rgba(255,255,255,0.8)':'#64748b'}}>訪問{s.visits}件 / ID開設{s.ids}件</div>
                      </div>
                      <div style={{textAlign:'right'}}>
                        <div style={{fontSize:'20px',fontWeight:'800',color:i<3?'#fff':'#059669'}}>¥{s.incentive.toLocaleString()}</div>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tbody>
                      {records.map(r=>(
                        <tr key={r.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'14px 10px'}}>
                            <div style={{display:'flex',gap:'4px',flexWrap:'wrap'}}>
                              {PRODUCTS.filter(p=>r[`result_${p.id}`]&&r[`result_${p.id}`]!=='-'&&r[`result_${p.id}`]!=='未提案').map(p=>{
          ...
          <div style={{background:'#fff',borderRadius:'16px',width:'100%',maxWidth:'500px',maxHeight:'90vh',overflow:'auto'}}>
            ...
            <div style={{padding:'16px 20px',display:'grid',gap:'14px'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                ...
                <div>
                  <label style={{fontSize:'13px',color:'#374151',fontWeight:'500',display:'block',marginBottom:'6px'}}>メール</label>
                  <input type="email" value={formData.raksul_email} onChange={e=>setFormData({...formData,raksul_email:e.target.value})} style={{width:'100%',padding:'12px',borderRadius:'8px',border:'1px solid #e2e8f0',fontSize:'14px',boxSizing:'border-box'}}/>
                </div>
              </div>
              ...
              {/* 各商材の結果 */}
              <div>
                <label style={{fontSize:'13px',color:'#374151',fontWeight:'500',display:'block',marginBottom:'8px'}}>各商材の結果</label>
                <div style={{display:'grid',gap:'12px'}}>
                  {PRODUCTS.map(p=>(
                    <div key={p.id} style={{padding:'14px',background:'#f8fafc',borderRadius:'10px',border:formData[`result_${p.id}`]&&formData[`result_${p.id}`]!=='-'?`2px solid ${RESULT_COLORS[formData[`result_${p.id}`]]?.text||p.color}20`:'1px solid #e2e8f0'}}>
                      {/* 商材名と結果ボタン */}
                      <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'10px',flexWrap:'wrap'}}>
                        <span style={{fontSize:'14px',color:p.color,fontWeight:'600',minWidth:'70px'}}>{p.name}</span>
                          ...
                          {RESULT_LIST.filter(r=>r!=='-').map(result=>(
                            <button 
                              ...
                              style={{
                                ...
                                borderRadius:'6px',
                                fontSize:'12px',
                                border:formData[`result_${p.id}`]===result?`2px solid ${RESULT_COLORS[result]?.text||'#64748b'}`:'1px solid #d1d5db',
                                background:formData[`result_${p.id}`]===result?RESULT_COLORS[result]?.bg||'#f3f4f6':'#fff',
                                color:formData[`result_${p.id}`]===result?RESULT_COLORS[result]?.text||'#374151':'#64748b',
                                cursor:'pointer',
                ...
                <div style={{marginTop:'16px'}}>
                  ...
                  <div style={{display:'grid',gap:'6px'}}>
                    {selectedProduct.videos.map((v,i)=>(
                      <a key={i} href={v.url} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px',background:'#fef3c7',borderRadius:'8px',textDecoration:'none',border:'1px solid #fde68a'}}>
                        <span style={{fontSize:'20px'}}>🎬</span>
                        <span style={{fontSize:'14px',fontWeight:'600',color:'#92400e'}}>{v.name}</span>
                      </a>
                    ))}
                  </div>
                </div>
