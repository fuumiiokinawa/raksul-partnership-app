...
const PRODUCT_DETAILS = [
  ...
  { id: 'meo', name: 'MEO対策', tagline: 'Googleマップ上位表示', color: '#7C3AED', icon: '📍', docUrl: MEO_DOC_URL, features: [{ label: 'セルフプラン', value: '1万円/月' },{ label: 'プロプラン', value: '4万円/月', highlight: true },{ label: '対策内容', value: '多数' },{ label: '他社比較', value: '安め', highlight: true }], target: '店舗ビジネス', salesPoint: '明確なニーズあり', incentive: '契約: ¥10,000' },
...
const MANUAL_SECTIONS = [
  { id: 'overview', title: '📱 概要', content: 'ラクスル提携商材の営業活動を記録・分析するツールです。' },
  { id: 'dashboard', title: '📊 集計', content: '商材別・担当者別・事務所別の分析ができます。CSV出力も可能です。' },
  { id: 'records', title: '📋 記録', content: '訪問記録の登録・編集・削除ができます。NGの場合はプルダウンから理由を選択します。' },
  { id: 'products', title: '📦 商材', content: '6つの提携商材の詳細情報を確認できます。' },
  { id: 'incentive', title: '💰 報酬', content: 'ID開設¥3,000、モール購入¥7,000、その他契約¥10,000' }
];
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const staffStats = STAFF_LIST.map(s => {
      ...
      return { name: s, visits: sr.length, ids: sr.filter(r => r.raksul_id_status === '開設済').length, incentive: sr.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(s => s.visits > 0);
    const officeStats = OFFICE_LIST.map(o => {
      const or = records.filter(r => r.office === o);
      return { name: o, visits: or.length, ids: or.filter(r => r.raksul_id_status === '開設済').length, incentive: or.reduce((sum,r) => sum + calcIncentive(r), 0) };
    }).filter(o => o.visits > 0);
      ...
      <header style={{background:'#fff',borderBottom:'1px solid #e2e8f0',padding:'12px 16px',position:'sticky',top:0,zIndex:100}}>
        <div style={{maxWidth:'1400px',margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'10px'}}>
          ...
          <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
            {[{key:'dashboard',label:'📊集計'},{key:'records',label:'📋記録'},{key:'products',label:'📦商材'},{key:'manual',label:'📖ヘルプ'}].map(t=>
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:'13px',minWidth:'800px'}}>
                    <thead>
                      <tr style={{background:'#f8fafc'}}>
                        {['日付','担当','企業名','事務所','ID','提案','結果','報酬','操作'].map(h=>
                    ...
                    <tbody>
                      {records.map(r=>(
                        <tr key={r.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'14px 10px'}}>{r.raksul_id_status==='開設済'&&<span style={{padding:'4px 8px',background:'#dcfce7',color:'#166534',borderRadius:'6px',fontSize:'11px'}}>開設済</span>}</td>
                          ...
                          <td style={{padding:'14px 10px'}}>
                            <div style={{display:'flex',gap:'4px',flexWrap:'wrap'}}>
                              {PRODUCTS.filter(p=>r[`result_${p.id}`]&&r[`result_${p.id}`]!=='-').map(p=>{
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'12px'}}>
              {PRODUCT_DETAILS.map(product=>(
                <div key={product.id} onClick={()=>setSelectedProduct(product)} style={{background:'#fff',borderRadius:'12px',border:'1px solid #e2e8f0',overflow:'hidden',cursor:'pointer'}}>
                  <div style={{background:`linear-gradient(135deg,${product.color},${product.color}dd)`,padding:'14px',color:'#fff'}}>
            ...
            <div style={{background:'#fff',borderRadius:'12px',border:'1px solid #e2e8f0',padding:'20px'}}>
              {MANUAL_SECTIONS.filter(s => s.id === manualSection).map(s => (
                <div key={s.id}>
                  ...
                  <p style={{fontSize:'14px',lineHeight:'1.8',color:'#475569'}}>{s.content}</p>
                </div>
              ))}
              <div style={{marginTop:'24px',padding:'16px',background:'#f8fafc',borderRadius:'10px'}}>
                <div style={{fontSize:'11px',color:'#64748b'}}>バージョン</div>
                <div style={{fontSize:'14px',fontWeight:'600'}}>{APP_VERSION}</div>
                <div style={{fontSize:'11px',color:'#64748b',marginTop:'4px'}}>最終更新: {LAST_UPDATED}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',zIndex:200}}>
          <div style={{background:'#fff',borderRadius:'16px',width:'100%',maxWidth:'500px',maxHeight:'90vh',overflow:'auto'}}>
            ...
            <div style={{padding:'16px 20px',display:'grid',gap:'14px'}}>
              ...
              {/* 提案商材 */}
              <div>
                <label style={{fontSize:'13px',color:'#374151',fontWeight:'500',display:'block',marginBottom:'8px'}}>提案商材</label>
                <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                  {PRODUCTS.map(p=>(
                    <button key={p.id} type="button" onClick={()=>setFormData({...formData,[`proposal_${p.id}`]:formData[`proposal_${p.id}`]==='○'?'-':'○'})} style={{padding:'10px 16px',borderRadius:'8px',border:formData[`proposal_${p.id}`]==='○'?`2px solid ${p.color}`:'1px solid #e2e8f0',background:formData[`proposal_${p.id}`]==='○'?`${p.color}15`:'#fff',color:formData[`proposal_${p.id}`]==='○'?p.color:'#64748b',fontSize:'13px',fontWeight:'500',cursor:'pointer'}}>{p.name}</button>
                  ))}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div style={{display:'grid',gap:'12px'}}>
                    {PRODUCTS.filter(p=>formData[`proposal_${p.id}`]==='○').map(p=>(
                      <div key={p.id} style={{padding:'14px',background:'#f8fafc',borderRadius:'10px'}}>
                        {/* 商材名と結果ボタン */}
                          ...
                          <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>
                            {RESULT_LIST.filter(r=>r!=='-').map(result=>(
                              <button 
                                ...
                                type="button" 
                                onClick={()=>{
                                  setFormData({...formData,[`result_${p.id}`]:formData[`result_${p.id}`]===result?'-':result,[`ng_${p.id}`]:result!=='NG'?'':formData[`ng_${p.id}`]});
